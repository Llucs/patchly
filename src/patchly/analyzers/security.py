...
class SecurityAnalyzer(BaseAnalyzer):
    def analyze(self, files: list[Path] | None = None, file_contents: dict[str, str] | None = None) -> list[ActionResult]:
        content_batches = []

        if file_contents:
            for path_str, text in file_contents.items():
                if text.strip():
                    content_batches.append(f"### {path_str}\n```\n{text[:3000]}\n```")
        elif files:
            for f in files:
                try:
