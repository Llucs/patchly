...
class Agent:
    ...
    def _run_review(self, web_content: str, memory_context: str) -> list[ActionResult]:
        llm_log("Running review analysis...")
        return self._analyze("review", web_content, memory_context)

    def _run_scan(self, memory_context: str) -> list[ActionResult]:
        llm_log("Running scan analysis...")
        return self._analyze("scan", "", memory_context)

    def _run_fix(self, web_content: str, memory_context: str) -> list[ActionResult]:
        llm_log("Running fix analysis...")
        try:
            check_safe()
        except SafeModeError:
            llm_log("Safe mode is enabled; no fixes will be applied.")
            return []
        return self._analyze("fix", web_content, memory_context)

    def _run_continuous(self, memory_context: str) -> list[ActionResult]:
        llm_log("Running continuous analysis...")
        return self._analyze("continuous", "", memory_context)

    def _run_command(self, web_content: str, memory_context: str) -> list[ActionResult]:
        llm_log("Running command analysis...")
        return self._analyze("command", web_content, memory_context)

    def _run_resolve(self, memory_context: str) -> list[ActionResult]:
        llm_log("Running resolve analysis...")
        return self._analyze("resolve", "", memory_context)

    def _analyze(self, mode: str, web_content: str, memory_context: str) -> list[ActionResult]:
        file_paths = list_project_files(self.config)
        file_contents = []
        for file_path in file_paths:
            try:
                file_contents.append(Path(file_path).read_text(encoding="utf-8", errors="replace"))
            except OSError:
                continue
        ...
        return run_analyzers(files=file_contents, context=context)

    def _write_report(self, results: list[ActionResult]) -> None:
        if not self.config.outputs.reports:
            return
        ...
        report_file = REPORTS_DIR / f"{self.config.mode}-{timestamp}.json"
